const handling = (app, db) => {
	...
	app.post('/api/insert/seance/non', (req, res) => {
		...
		const sqlInsert =
			'INSERT INTO seances (id_prof, id_module, nom_filiere, niveau) VALUES (?,?,?,?)';

		db.query(
			sqlInsert,
			[id_prof, id_module, nom_filiere, niveau],
			(err, result) => {
				if (err) {
					console.log(err);
					res.send(false);
	...
	app.post('/api/insert/seance/oui', (req, res) => {
		...
		const sqlInsert =
			'INSERT INTO seances (id_prof, id_ss_module, nom_filiere, niveau) VALUES (?,?,?,?)';

		db.query(
			sqlInsert,
			[id_prof, id_ss_module, nom_filiere, niveau],
			(err, result) => {
				if (err) {
					console.log(err);
					res.send(false);
